...
def parse_description(
    description: Optional[str],
    response_payload: Optional[Any] = None,   # ✅ accept both dict or model
) -> dict:
    ...
    # ✅ NEW: handle both dict and model
    status = None
    ...
    if response_payload:
        if isinstance(response_payload, dict):
            ...
            status = getattr(response_payload, "status", None)

    if status:
        # dict case
        if isinstance(status, dict):
            if not code_val:
                code_val = status.get("code") or status.get("errorcode")
            if not reason_val:
                reason_val = status.get("errorcode")
            if not message_val:
                message_val = status.get("errormessage")

        # model case
        else:
            if not code_val and getattr(status, "code", None):
                code_val = status.code.strip()
            if not code_val and getattr(status, "errorcode", None):
                code_val = status.errorcode.strip()
            if not reason_val and getattr(status, "errorcode", None):
                reason_val = status.errorcode.strip()
            if not message_val and getattr(status, "errormessage", None):
                message_val = status.errormessage.strip()
...
def build_fallout_prompt(
    ...
    masked_response_payload = (
        mask_payload(request.responsePayload)
        if request.responsePayload else None
    )

    masked_request_payload = (
        mask_payload(request.requestPayload)
